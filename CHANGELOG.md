...
- Fixed the `sf-toolbox` conflict detector reporting Homebrew-managed tools as standalone binaries and telling the user to `sudo rm` them, which broke Homebrew's link bookkeeping. The Omarchy mise-wrapper check ran before the package-manager path checks and grepped binaries for `mise use -g`, a string glab embeds in its own upgrade help. The wrapper check now runs after the Homebrew and pacman checks, is scoped to real non-symlink files under `~/.local/bin`, and uses `grep -I` so binaries never match
