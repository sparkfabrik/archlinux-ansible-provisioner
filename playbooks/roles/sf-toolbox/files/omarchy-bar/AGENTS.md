...
- **One GraphQL query, not a fan-out.** The per-project data (metadata, open issues, open merge requests, pipelines, and the last note timestamp per item) comes from a single `glab api graphql` call addressing the projects by global id. The REST path it replaced cost five calls per project, 40 in total against 11 now, and a cold build went from 13.5s to about 7s. Keep it one query: adding a per-project REST call puts the fan-out back. `pipelines` takes no author argument, so it is fetched wide and filtered to the developer in jq, and GraphQL statuses are upper case and get lowered to keep the contract the UI reads. `gql()` degrades to an empty document, so an unavailable field never empties the payload.
- **Comment timestamps come from the item.** GitLab does not move `updated_at` when someone comments, so ordering by real activity needs the note time. GraphQL returns `notes(last: 1)` per item, which replaced a project-wide comment event feed and its inference. Do not reintroduce that feed.
- **Drop the noise the agent creates.** Every issue or merge request an agent writes for the developer carries an "on behalf of @user" header, which GitLab reads as a mention and turns into a todo addressed back at them: on a real account, 76 of 79 mentions. `attention` therefore drops todos the developer authored themselves, but only for `mentioned` and `directly_addressed`. A self assignment is deliberate work and a failed build on a pipeline they started is real signal, so those stay. `totals.self_mentions_dropped` reports the count: a list that silently shrinks is worse than a noisy one.
...
Env overrides for tests and non-standard layouts: `SF_TOOLBOX_DIR`, `SF_SPARKDOCK_DIR`, `SF_HTTP_PROXY_DIR`, `SF_AGENTS_DIR`, `SF_STATUS_FETCH_TIMEOUT`, `SF_GITLAB_CLIENT_GROUPS`, `SF_GITLAB_MAX_PROJECTS` (how many projects get the GraphQL enrichment, default 6).
